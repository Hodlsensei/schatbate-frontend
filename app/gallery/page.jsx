...
const NAMES = [
  ...
  "GoldenHour","MidnightSky","SapphireGlow","AmberWave","PeachBlossom","DiamondCut",
];
...
function generatePhotoBatch(count = 12) {
...
/* ── Photo Card — natural height, no forced aspect ratio ── */
function PhotoCard({ item }) {
  ...
  return (
    ...
    >
      {/* Image at natural height — drives the card height */}
      <img
        src={item.src}
        alt={item.name}
        style={{
      ...
      <div style={{ position: "absolute", top: 7, right: 7 }}>
        <div style={{
          width: 22, height: 22, background: "rgba(0,0,0,0.55)",
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{
        ...
        paddingBottom: "56.25%", background: "#222",
        transform: hov ? "scale(1.02)" : "scale(1)",
        transition: "transform .2s, box-shadow .2s",
      ...
      <div style={{ display: "flex", gap: 8, padding: "8px 2px 4px", alignItems: "flex-start" }}>
        <div style={{
          ...
          {item.isLive && (
            <div style={{
              ...
              borderRadius: 3, fontFamily: FONT, whiteSpace: "nowrap",
            }}>LIVE</div>
          )}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            fontFamily: FONT, lineHeight: 1.35, marginBottom: 3,
...
export default function GalleryPage() {
  ...
  const [photos, setPhotos] = useState(() => generatePhotoBatch(16));
  ...
  const loadMore = useCallback(() => {
    ...
    setTimeout(() => {
      if (activeTab === "Photos") {
        setPhotos(prev => [...prev, ...generatePhotoBatch(12)]);
      } else {
  ...
  return (
    <div style={{ background: isVideos ? "#fff" : "#111", minHeight: "100vh", fontFamily: FONT }}>
      <style>{`
      ...
      <div style={{
        ...
        background: isVideos ? "#fff" : "#111",
        borderBottom: isVideos ? "1px solid #eee" : "none",
        ...
        <div style={{ position: "absolute", right: 20 }}>
          <svg width={20} height={20} viewBox="0 0 24 24" fill={isVideos ? "#666" : "#888"}>
      ...
      {/* PHOTOS — 4-col CSS masonry (natural image heights) */}
      {activeTab === "Photos" && (
        <div style={{ padding: 8, columns: 4, columnGap: 8 }}>
